...
class Person:
    ...
    def __init__(self, tracker, x, y, w, h):
    ...
    def set_postions(self, x, y, w, h): # TODO w, h niekoniecznie potrzebne
        self.xB = x + w
        self.yB = y + h
        self.xA = x
        self.yA = y
...
def show(filename):
    ...
    out = cv2.VideoWriter('outanalyze.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (frame_width, frame_height))
        ...
        if len(idxs) > 0:
            ...
            for i in idxs.flatten():
                ...
                for person in people:
                    if person.check_if_its_me(x, y, x + w, y + h):
                        found = True
                if not found:
                    tracker = cv2.TrackerKCF_create()
                    tracker.init(frame, (x, y, w, h))
                    people.append(Person(tracker, x, y, w, h))
